CREATE DATABASE IF NOT EXISTS qycms_coupon_db;

-- 创建 coupon_template 数据表
DROP TABLE IF EXISTS `qycms_coupon_db`.`coupon_template`;

CREATE TABLE IF NOT EXISTS `qycms_coupon_db`.`coupon_template` (
...
DROP TABLE if exists  `qycms_coupon_db`.`coupon` ;
CREATE TABLE IF NOT EXISTS `qycms_coupon_db`.`coupon` (
...
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=utf8mb4 COMMENT='领到手的优惠券';